...
das Buch lebt, und **eine einzige Datei, in der die Bewertungslogik des Website
Standards steht**.

Der Grund: Die 100 Punkte stehen heute an drei verschiedenen Stellen im Code
(`AuditReport.jsx`, `WebsiteChecklist.jsx`, `audit-widget.html`). Wenn du eine davon
...
`shared/website-standard.json` als verbindliche Wahrheit, und einen Befehl
...
Lies zusätzlich frontend/src/components/WebsiteChecklist.jsx und extrahiere je Item:
...
SCHRITT 2
Lege die Datei shared/website-standard.json an. Struktur:
{
  "version": "2026.1",
  "released": "2026-09-01",
  "total_points": 100,
...
SCHRITT 4
Lege scripts/check-website-standard.js an. Das Skript:
- liest shared/website-standard.json
...
Trage in package.json unter scripts ein: "check:standard": "node scripts/check-website-standard.js"
...
SCHRITT 6
Verifiziere:
node -e "const d=require('./shared/website-standard.json'); console.log('Summe:', d.categories.reduce((a,c)=>a+c.max,0))"
npm run check:standard
ls -R buch/

SCHRITT 7
git add -A
git commit -m "Add book manuscript structure and Website Standard single source of truth"
...
```
Add book manuscript structure and Website Standard single source of truth
```
