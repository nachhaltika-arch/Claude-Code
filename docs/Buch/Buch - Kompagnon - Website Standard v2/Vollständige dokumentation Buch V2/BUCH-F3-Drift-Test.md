...
audit_criteria.py            ← maßgeblich · Summe bereits bewacht ✅
        │
        ├── website-standard.json          (erzeugt, F2)          — neu bewachen
        ├── docs/Buch/generiert/*.md        (erzeugt, F2)          — neu bewachen
        ├── utils/websiteStandard.js       (handgepflegt)          — neu bewachen
...
grep -n -A10 "export const STUFEN" kompagnon/frontend/src/utils/websiteStandard.js
...
Zweite neue Datei: `kompagnon/frontend/src/utils/websiteStandard.test.js`

Sie prüft die beiden handgepflegten Stellen gegen `website-standard.json`:
...
1. **`STUFEN` in `websiteStandard.js`** muss den Stufen aus `website-standard.json` entsprechen — gleiche Schwellen, gleiche Namen, gleiche Reihenfolge.
...
Dasselbe für den Frontend-Test: eine Schwelle in `websiteStandard.js` auf 84 ändern, Test muss rot werden, zurückändern, grün.
...
| Schnittstelle liefert ihn aus | `standard-export.py` erzeugt `website-standard.json` |
| Frontend hat eine Adresse dafür | `websiteStandard.js` und Widget lesen dieselben Zahlen — vom Test geprüft |
...
git add kompagnon/backend/tests/test_buch_stimmt_mit_code.py \
        kompagnon/frontend/src/utils/websiteStandard.test.js \
        .github/workflows/
