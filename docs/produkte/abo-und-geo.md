...
| 3 | ✅ **erledigt 25.08.2026 — der Wirkungsbericht steht** (`services/geo_wirkungsbericht.py`, Endpunkt `GET /api/geo/{id}/wirkungsbericht`). Er vergleicht zwei Größen **getrennt**: die Lesbarkeit (unser Werk) und die Nennungen (nicht unser Werk) — beides in eine Zahl zu pressen verkaufte eine Wirkung, die niemand zusichern kann. Vor dem 57. Tag sagt er, dass es zu früh ist; bei nur einem Messpunkt benennt er die Lücke, statt eine Veränderung gegen nichts zu rechnen. Ursprünglich: Seit dem 25.08. misst `services/ki_sichtbarkeit.py` die Nennung in vier KI-Systemen und schreibt einen Verlauf fort. Was fehlt, ist die Auswertung „nach 60 Tagen" gegen den Stand bei Auslieferung — eine Abfrage auf vorhandene Daten, kein neues Verfahren |
