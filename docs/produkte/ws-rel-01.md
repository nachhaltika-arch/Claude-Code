# PRODUKTDATENBLATT · WS-REL-01
## WEBSPRINT RELAUNCH


> ## ✅ Nachgeprüft am 23.08.2026 — die beiden Techniksperren fallen
>
> Der Freigabestatus nennt **L2** (G1-Schwelle nicht belegbar, weil
> `PAGESPEED_API_KEY` fehle) und **L3** (Score-Schwellen zwischen Backend und
> Frontend verschieden). Beides am laufenden System gemessen, beides trifft
> **nicht** zu:
>
> - **L2 hinfällig.** Der Schlüssel ist auf dem Produktivdienst gesetzt und
>   arbeitet: Protokoll vom 22.08.2026, Aufruf gegen
>   `pagespeedonline/v5/runPagespeed` mit `HTTP/1.1 200 OK`. Die 18 Punkte
>   fließen ein, der Score ist vollständig erhebbar. Offen bleibt allein die
>   **Zahl**, die G1 zusagt — sie ist noch nicht festgelegt.
>   *(Nebenbefund: der Schlüssel steht im Klartext im Protokoll → L-98.)*
> - **L3 hinfällig.** Backend und Frontend führen dieselben vier Stufen,
>   **95 / 85 / 70 / 50**, Wert für Wert gleich. Die zuerst notierte
>   Abweichung entstand durch eine Suche, die „Platin" nicht mitzählte.
>
> **Damit ist dieses Paket verkaufbar** — der Freigabestatus nannte genau
> diese beiden Bedingungen. Es steht seit dem 23.08.2026 im Katalog
> (`websprint_relaunch`, 3.500 € netto / 4.165 € brutto) und ist auf `live`
> geschaltet.

| | |
|---|---|
| Artikelnummer | WS-REL-01 |
| Systemwert `product_type` | `websprint_relaunch` |
| Angebotsbezeichnung | Websprint Relaunch — Fertigstellung nach Homepage-Standard |
| Preis | **3.500 € netto** (Festpreis) |
| Umsatzsteuer | 19 % |
| Zahlungsbedingung | **Z1** (50/50) |
| Bauzeit | **14 Kalendertage** |
| Mitwirkung | **M1, M2, M3, M4, M5**, bei Migration zusätzlich M9 |
| Garantien | **G1, G2** (G2 gedeckelt auf 1.000 €) |
| Abgrenzung | **A1–A15** vollständig |
| Freigabestatus | 🟠 verkaufbar nach Behebung L2 und L3 |

---

## 1. Eignung / Zielkunde
Betrieb mit **vorhandener Website**, deren Inhalte im Kern verwendbar sind. Typischer Eingangs-Score **35–70 Punkte**.

**Passt, wenn:** Seite älter als 4 Jahre · nicht mobiloptimiert · fehlendes oder gemischtes SSL · veraltete Rechtstexte · Ladezeit über 4 Sekunden · keine erkennbaren Kontaktwege.

**Passt nicht, wenn:** keine Website vorhanden · Umfirmierung oder Generationswechsel · Recruiting ist Hauptmotiv · mehr als 6 Seiten benötigt. → WS-NEU-01 oder WS-SYS-01.

---

## 2. Leistungsverzeichnis (angebotsfertig)

| Pos. | Leistung | Menge |
|---|---|---|
| 1.1 | Eingangsaudit nach Homepage-Standard, 100 Punkte in 8 Kategorien, schriftlich dokumentiert | 1 |
| 1.2 | Strukturabgleich und Seitenplan auf Basis der bestehenden Website | 1 |
| 2.1 | Aufbau der Seiten im KOMPAGNON-Komponentensystem, responsiv für Desktop, Tablet und Smartphone | bis 6 Seiten |
| 2.2 | Übernahme und redaktionelle Überarbeitung der vorhandenen Texte (Kürzung, Gliederung, Ansprache) | bis 6 Seiten |
| 2.3 | Aufbereitung des vom Kunden gelieferten Bildmaterials (Zuschnitt, Komprimierung, moderne Bildformate, Alternativtexte) | bis 30 Bilder |
| 2.4 | Kontaktformular mit Spam-Schutz und Empfangsbestätigung | 1 |
| 2.5 | Einbindung der vom Kunden gelieferten Rechtstexte | 1 |
| 2.6 | Grundlagen der Barrierefreiheit: Kontrastwerte, Tastaturbedienbarkeit, semantische Struktur, Alternativtexte | inkl. |
| 3.1 | Technische Grundoptimierung: Ladezeit, Bildformate, Metadaten, strukturierte Auszeichnung des Betriebs | inkl. |
| 3.2 | Einrichtung des Hostings, SSL, Weiterleitungen der bisherigen Adressen | 1 |
| 3.3 | Umstellung der Domain (DNS) auf die neue Seite, Begleitung der Umschaltung | 1 |
| 4.1 | Abnahmeaudit nach Homepage-Standard mit schriftlichem **Abnahmeprotokoll** je Kategorie | 1 |
| 4.2 | Einweisung des Ansprechpartners, 30 Minuten, per Videokonferenz | 1 |
| 4.3 | Übergabe aller Zugänge und Dateien | 1 |

**Nicht enthalten:** A1–A15 (siehe Baukasten).

---

## 3. Optionale Zusatzpositionen

| Art.-Nr. | Zusatzleistung | Preis netto |
|---|---|---|
| ZUS-SEITE | Jede weitere Seite über 6 hinaus | 290 € |
| ZUS-TEXT | Texterstellung statt Übernahme, je Seite | 350 € |
| ZUS-FOTO | Fotoproduktion vor Ort, halber Tag inkl. Bearbeitung | 890 € |
| ZUS-RECHT | Vermittlung Rechtstexte über Partnerkanzlei | ab 250 € |
| GEO-01 | GEO/GAIO Add-on | 1.200 € 🔴 |
| ABO-BAS | Pflege Basic ab Abnahme | 79 €/Mon. |
| ABO-PRO | Pflege Pro ab Abnahme | 149 €/Mon. |

🔴 GEO-01 derzeit nicht anbietbar, siehe Blocker L1.

---

## 4. Ablauf und Termine

| Phase | Inhalt | Dauer |
|---|---|---|
| 0 | Auftragserteilung, Anforderung der Mitwirkungsleistungen | — |
| 1 | Eingang M1–M5 → **Fristbeginn** | Tag 0 |
| 2 | Struktur und Rohaufbau | Tag 1–6 |
| 3 | Inhalte, Bilder, Feinaufbau | Tag 7–10 |
| 4 | Kundenvorschau, eine Korrekturschleife | Tag 11–12 |
| 5 | Technische Optimierung, Abnahmeaudit | Tag 13 |
| 6 | DNS-Umstellung, Go-Live, Protokoll, Einweisung | Tag 14 |

**Enthalten ist eine Korrekturschleife.** Jede weitere Schleife: 290 € netto.

---

## 5. Angebotstext (Baustein zum Einsetzen)

> **Websprint Relaunch — Fertigstellung nach Homepage-Standard**
>
> Ihre bestehende Website erreicht im Eingangsaudit [SCORE] von 100 Punkten. Wir bringen sie in 14 Tagen auf Abnahmeniveau: gleiche Inhalte, neu aufgebaut, technisch und rechtlich auf Stand, mit dokumentierter Abnahme.
>
> Leistungsumfang: bis zu 6 Seiten, Übernahme und Überarbeitung Ihrer Texte, Aufbereitung Ihres Bildmaterials, Kontaktformular, Einbindung Ihrer Rechtstexte, technische Optimierung, Hosting-Einrichtung, Umstellung Ihrer Domain, Abnahmeaudit mit schriftlichem Protokoll sowie eine Einweisung.
>
> **Festpreis 3.500 € netto.** Bauzeit 14 Kalendertage ab Vorliegen Ihrer Mitwirkungsleistungen. Eine Korrekturschleife ist enthalten.

---

## 6. Interne Kalkulation (nicht ins Angebot)

| Position | Ansatz |
|---|---|
| Audit und Seitenplan | 2 h |
| Aufbau 6 Seiten | 12 h |
| Text- und Bildaufbereitung | 6 h |
| Technik, Deployment, DNS | 3 h |
| Abnahme, Protokoll, Einweisung | 2 h |
| Projektleitung und Kommunikation | 4 h |
| **Summe** | **~29 h** |
| Kalkulatorischer Stundensatz | ~120 €/h |
| Direkte Fremdkosten | Hosting, Lizenzanteil, API-Kosten |

**Deckungsbeitrag ist tragfähig, solange die Korrekturschleifen begrenzt bleiben.** Die zweite kostenlose Schleife kostet rund 4 Stunden und damit spürbar Marge — deshalb steht die Begrenzung im Angebot.

---

## 7. Offene Punkte vor Verkaufsstart

| # | Punkt | Verantwortung |
|---|---|---|
| 1 | 🔴 **G1 ohne belegbare Schwelle nicht verwendbar** — PageSpeed-Key setzen, dann 5 Referenzmessungen | Technik |
| 2 | 🔴 Score-Schwellen zwischen Frontend und Backend vereinheitlichen | Technik |
| 3 | Mitwirkungskatalog und Fristregelung in die AGB übernehmen, wortgleich | Recht |
| 4 | Verzugspauschale und Deckelung anwaltlich prüfen | Recht |
| 5 | Bildlizenz-Haftung: Klausel, dass der Kunde die Rechte an M3 zusichert | Recht |
