...
| L-59 | **Wir pruefen bei Kunden, was wir bei uns selbst nicht fuehren: die Rechtsgrundlage.** `audit_collectors.py:255` sucht auf fremden Seiten nach „Art. 6", `netlify_service.py:531` schreibt sie in erzeugte Seiten, `pdf_generator.py:427` druckt eine Spalte dafuer — fuer die **eigenen** Leaddaten steht sie an keiner Stelle. 79 Felder am Lead, keines nennt sie. Dieselbe Form wie L-17: Wir verkaufen die Pruefung und bestehen sie selbst nicht. **Korrektur zweier eigener Vermutungen vom 21.08.:** Die Datensatzquelle fehlt **nicht** — `lead_source` gibt es, wird an fuenf Stellen gesetzt und im Betriebe-Filter gelesen; offen ist ihr **Wortschatz**, der ungeregelt ist (`stripe_checkout`, `domain_import`, `csv_import`, `embed_audit`, `llm_landing`, `manual`, dazu freier Text aus Kampagnen wie `HWK-Muenchen`) — dieselbe Klasse wie der Statuswortschatz vom 19.08. Und die Lifecycle-Phase ist seit `a7e9cc5` da, der Punkt war bereits erledigt | S | `database.py` Lead (79 Felder), `services/audit_collectors.py:255` |
...
| L-29 | **Preise an vier Stellen — und sie sind bereits auseinandergelaufen.** Nachgemessen 19.08.: Premium steht in `payments.py` mit **2.800**, in `OfferTab.jsx` und `PricingSection.jsx` mit **2.500**; Kompagnon in `payments.py` mit **2.000**, in `Landing.jsx` mit **3.500**. Die `products`-Tabelle ist die eigentliche Quelle und wird von Hand gepflegt. **Erledigt:** Die feste Liste `PACKAGE_NAMES` ist weg — sie stand im **Text der Kundenmail** nach dem Kauf, also in einem Schreiben, das der Kunde aufhebt. Name und Betrag kommen jetzt aus derselben Zeile; ein unbekanntes Produkt bekommt die nackte Kennung statt eines erfundenen Preises. **Offen: die drei Frontend-Stellen.** Welcher Preis gilt, steht in der Datenbank und ist Davids Entscheidung — ich waehle keinen aus | S | `test_paketbezeichnung.py` |
