...
@router.post("/{project_id}/sitemap-suggest")
async def suggest_sitemap(
    project_id: int,
    db: Session = Depends(get_db),
    _=Depends(require_any_auth),
):
    """Claude schlaegt optimale Sitemap basierend auf gecrawlten Seiten vor."""
    import os, httpx, json, re
    ...
    crawled = db.execute(
        text("SELECT url, title, h1, word_count FROM website_content_cache WHERE customer_id=:lid ORDER BY word_count DESC LIMIT 20"),
        {"lid": lead_id},
    ).fetchall()

    if not crawled:
        raise HTTPException(400, "Keine gecrawlten Seiten — zuerst Analyse-Zentrale ausfuehren")

    briefing = db.execute(
        text("SELECT gewerk, leistungen, einzugsgebiet FROM briefings WHERE lead_id=:lid LIMIT 1"),
        {"lid": lead_id},
    ).fetchone()
    ...
    for row in crawled:
        ...
        try:
            ...
            path = url
        seiten_info.append(f"- /{path}: {h1 or title} ({wc or 0} Woerter)")
    ...
    prompt = (
        f"Du entwirfst eine optimale Website-Sitemap fuer einen {briefing[0] if briefing else 'Handwerksbetrieb'}.\n\n"
        f"Bestehende Seiten:\n{chr(10).join(seiten_info)}\n\n"
        f"Leistungen: {briefing[1] if briefing else '—'}\n"
        f"Region: {briefing[2] if briefing else '—'}\n\n"
        "Erstelle eine optimale Sitemap fuer die NEUE Website.\n"
        "Pflichtseiten: Startseite, Kontakt, Ueber uns, Impressum, Datenschutz.\n"
        "Leistungsseiten fuer jede Hauptleistung einzeln. Maximal 12 Seiten.\n\n"
        "Antworte NUR als JSON-Array:\n"
        '[{"page_name":"Startseite","page_type":"startseite","slug":"","ziel_keyword":"...","zweck":"..."}]'
    )
    ...
    try:
        ...
        text_resp = re.sub(r'^```json\s*', '', text_resp)
        text_resp = re.sub(r'\s*```$', '', text_resp)
        pages = json.loads(text_resp)
        return {"suggested_pages": pages, "count": len(pages)}
    except Exception as e:
        raise HTTPException(500, f"Fehler: {str(e)[:200]}")
