...
(function () {
  ...
  * { box-sizing: border-box; }
  ...
  .kpg-mailnote-klein { margin: 10px 0 0; font-size: 13px;
    color: var(--kpg-muted); }
  /* ── Schritt 2: der Bericht wird angefordert (21.09.2026) ──
     Derselbe Kasten wie die Mailnote — bewusst, denn er steht an derselben
     Stelle und wird von ihr abgeloest, sobald das Formular weg ist. Ein
     anderer Rahmen an dieser Stelle laese den Wechsel wie einen Sprung
     aussehen. */
  .kpg-bericht {
    background: #E0F4F8; border-left: 4px solid var(--kpg-teal);
    border-radius: 0 7px 7px 0; padding: 18px 18px 16px;
  ...
  .kpg-bericht-kopf {
    ...
    color: var(--kpg-dark); margin: 0 0 6px;
  }
  .kpg-bericht-text {
    font-size: 14px; line-height: 1.55; color: var(--kpg-text-60);
    margin: 0 0 14px;
  }
  /* Im Kasten sitzen die Felder auf getoenten Grund — weiss statt der
     Kartenfarbe, sonst verschwindet der Rahmen im Hintergrund. */
  .kpg-bericht .kpg-input { background: #fff; }
  .kpg-bericht .kpg-field:last-of-type { margin-bottom: 0; }
  .kpg-input:focus {
...
`;
  ...
  /* Die Nummer der Anfrage aus Schritt 1. Sie wird erst in Schritt 2
     gebraucht — als Ereigniskennung des Leads, damit Browser und Serverweg
     dasselbe Ereignis melden und Meta es nicht doppelt zaehlt. */
  var anfrageKennung = '';
  ...
  /* **Die Ableitung „Website aus E-Mail-Domain" ist fort** (21.09.2026).
     Sie fuellte das Adressfeld aus dem, was hinter dem @ stand — und setzte
     damit voraus, dass die E-Mail **vor** der Adresse eingegeben wird. Im
     zweistufigen Formular ist die Reihenfolge umgekehrt: Wenn die Adresse
     gefragt wird, gibt es keine E-Mail, und wenn die E-Mail gefragt wird,
     ist die Adresse laengst geprueft. Eine Ableitung in diese Richtung
     wuerde ein ausgewertetes Feld ueberschreiben.

     Mit ihr fallen `urlBearbeitet` und die Freemail-Liste weg; beide hatten
     keinen anderen Verwender.
  ...
  /* ── Submit: Schritt 1 ──
     **Hier wurden bis zum 21.09.2026 vier Werte gelesen**: Adresse, E-Mail,
     Telefon und Haekchen. Drei davon sind an das Ergebnis gewandert; was
     bleibt, ist die Adresse. Der Kommentar ueber die Reihenfolge („lesen,
     solange das Formular noch im Dokument steht") gilt dort weiter und
     steht jetzt an `berichtAnfordern`. */
  ...
  function startAudit(cleanUrl) {
    ...
    fetch(API_BASE + '/api/widget/audit', {
      ...
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        /* **Ohne Adresse** (21.09.2026). Der Endpunkt nimmt sie weiterhin
           entgegen — das Feld bleibt in der Schnittstelle, weil aeltere
           eingebettete Fassungen des Widgets noch eine schicken. Diese
           Fassung schickt keine: Der Abschluss liegt in Schritt 2. */
    ...
    .then(function (start) {
      ...
      /* **Hier stand `meldeLead`, und das war bis zum 21.09.2026 richtig:
         Wer hier ankam, hatte seine Adresse schon hergegeben.** Jetzt nicht
         mehr — an dieser Stelle ist nur eine Analyse angestossen, niemand
         hat sich zu erkennen gegeben. Der Lead entsteht in
         `berichtAnfordern`, und er wird auch dort gemeldet.

         Wer das uebersieht, zaehlt jeden Besucher als Lead und rechnet die
         Kosten je Abschluss um den Faktor der Abbrecher schoen. */
      anfrageKennung = start.request_id || '';
      pollAudit(start.poll_token, cleanUrl, msgTimer);
    })
    .catch(function (err) {
  ...
  function pollAudit(pollToken, cleanUrl, msgTimer) {
    var attempts = 0;
      ...
      fetch(API_BASE + '/api/widget/teaser/' + encodeURIComponent(pollToken))
        ...
        .then(function (d) {
          if (d.status === 'completed') {
            clearInterval(poll); clearInterval(msgTimer);
            renderResult(d, cleanUrl, pollToken);
  ...
  /* ── Schritt 2: den ausfuehrlichen Bericht anfordern ──
     (Entwurf David „Formular-Zustaende", 21.09.2026)

     **Was sich hier verschiebt, ist nicht die Optik, sondern der Abschluss.**
     Bis heute war die Anfrage aus Schritt 1 der Lead: Wer den Knopf drueckte,
     hatte seine Adresse schon eingetippt. Jetzt kommt der Punktwert ohne
     Gegenleistung, und die Adresse wird erst hier gefragt — an der Stelle,
     an der der Besucher weiss, was der Bericht ihm bringt.

     Drei Dinge haengen daran und sind anderswo nachgezogen:
     * `POST /api/widget/audit` heisst „Analyse gestartet", nicht mehr „Lead".
     * Der Lead — Browser wie Serverweg — entsteht in diesem Formular.
     * Ohne Adresse geht keine Mail hinaus, auch wenn die Analyse fertig wird.

     Die Felder sind dieselben wie vorher in Schritt 1, mit denselben Texten:
     Was rechtlich traegt, traegt unabhaengig davon, an welcher Stelle es
     steht. Nur der Hinweis unter der E-Mail ist fort — „Dorthin schicken wir
     Ihren Bericht" stand ueber dem Kasten schon als Ueberschrift. */
  function berichtFormularHtml() {
    var umfang = CRITERIA_COUNT
      ? 'zu allen ' + esc(CRITERIA_COUNT) + ' Kriterien'
      : 'zu jedem gepruefen Kriterium';
    return '' +
      '<div class="kpg-bericht" id="kpg-bericht">' +
        '<p class="kpg-bericht-kopf">Den ausführlichen Bericht bekommen</p>' +
        '<p class="kpg-bericht-text">Der vollständige Befund ' + umfang +
          ' — was Punkte kostet und was zuerst zu tun ist — kommt per ' +
          'E-Mail, mit PDF zum Weitergeben.</p>' +
        '<form id="kpg-bericht-form">' +
          '<div class="kpg-field">' +
          ...
          '<div class="kpg-field">' +
            ...
            '<p class="kpg-hint">Wenn Sie eine Nummer eintragen, rufe ich Sie ' +
              'heute kurz an und nenne Ihnen den wichtigsten Befund. Kein ' +
              'Verkaufsgespräch. — David Väth</p>' +
          '</div>' +
          '<label class="kpg-consent" for="kpg-consent">' +
            '<input type="checkbox" id="kpg-consent" />' +
            '<span>Zeigen Sie mir per E-Mail, wie ich die gefundenen Punkte ' +
              'abstelle. Bis zu fünf E-Mails, Abmeldung jederzeit.</span>' +
          '</label>' +
          '<details class="kpg-consent-mehr">' +
            '<summary>Details</summary>' +
            '<p><strong>Ihren Analysebericht bekommen Sie in jedem Fall</strong> — ' +
            'dafür ist kein Häkchen nötig. Mit Häkchen zusätzlich bis zu fünf ' +
            'E-Mails, die zeigen, wie sich die gefundenen Punkte abstellen ' +
            'lassen. Abmeldung über den Link in jeder E-Mail. Ihre ' +
            'E-Mail-Adresse wird nicht an Meta oder andere Werbenetzwerke ' +
            'übermittelt. Verantwortlich: Kompagnon Communications BP GmbH, ' +
            'Marienfelder Straße 52, 56070 Koblenz.</p>' +
          '</details>' +
          '<button class="kpg-btn" id="kpg-bericht-submit" type="submit">' +
            'Bericht anfordern</button>' +
          '<p class="kpg-foot" id="kpg-bericht-fuss">Sie bekommen erst eine ' +
            'Bestätigungs-Mail, dann den Bericht — bitte nur die eigene ' +
            'Adresse eintragen</p>' +
        '</form>' +
      '</div>';
  }

  /* Meldet die Angaben nach und loest damit die Mailstrecke aus.

     **Die Werte muessen gelesen werden, solange das Formular im Dokument
     steht.** Gleich darunter wird der Kasten durch die Mailnote ersetzt;
     danach sind Haekchen und Telefonfeld weg, und jede Zustimmung waere
     verloren. Derselbe Fehler hat das Haekchen schon einmal gekostet. */
  function bindeBerichtFormular(pollToken) {
    var f = $('kpg-bericht-form');
    if (!f) return;
    f.addEventListener('submit', function (e) {
      ...
      fetch(API_BASE + '/api/widget/bericht-anfordern/' + encodeURIComponent(pollToken), {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          email: email,
          consent_marketing: !!(consentEl && consentEl.checked),
          ...
          /* Die Klickkennungen stehen **nicht** an der Anfrage (L-192: dort
             wird nur das Ob vermerkt, nie die Kennung). Ohne sie kann der
             Serverweg den Lead niemandem zuordnen — also gehen sie hier
             noch einmal mit. */
          fbclid: FBCLID,
          fbc: FBC,
          fbp: FBP,
      ...
      .then(function (r) {
        return r.json().then(function (body) {
          if (!r.ok) throw new Error(body.detail || 'Das hat gerade nicht geklappt.');
          return body;
        });
      })
      .then(function () {
        /* **Jetzt** ist es ein Lead — nicht beim Start der Analyse. */
        meldeLead('kpg-widget-' + anfrageKennung);
        zeigeAbschluss(pollToken, email);
      })
      .catch(function (err) {
        knopf.disabled = false;
        knopf.textContent = 'Bericht anfordern';
        var fuss = $('kpg-bericht-fuss');
        if (fuss) {
          fuss.textContent = (err && err.message) ||
            'Das hat gerade nicht geklappt — bitte noch einmal versuchen.';
          fuss.style.color = '#B3261E';
        }
        postHeight();
      });
    });
  ...
  /* Das Formular weicht dem Hinweis auf die Bestaetigungs-Mail. Beide sitzen
     im selben Kasten, damit der Wechsel nicht wie ein Sprung wirkt. */
  function zeigeAbschluss(pollToken, email) {
    var kasten = $('kpg-abschluss');
    if (!kasten) return;
    kasten.innerHTML =
      '<div class="kpg-mailnote" id="kpg-mailnote">' +
        mailnoteHtml('laeuft', email) +
      '</div>';
    beobachteBestaetigung(pollToken, email);
    postHeight();
  }

  function renderResult(data, cleanUrl, pollToken) {
    ...
    card.innerHTML =
      '<div>' +
        ...
        '</div>' +
        blockerHtml +
        /* **Der eine naechste Schritt.** Bis zum 21.09.2026 stand hier der
           Hinweis auf die Bestaetigungs-Mail, weil die Adresse zu diesem
           Zeitpunkt schon vorlag. Jetzt steht hier das Formular, das sie
           erfragt — und erst nach dem Absenden derselbe Hinweis, an
           derselben Stelle, im selben Kasten.

           Zwei Mails bleiben es: erst die Bestaetigung der Adresse, dann der
           Bericht. Ohne den Klick kommt kein Bericht — das ist kein Umweg,
           sondern der Grund, warum eine fremd eingetragene Adresse nie eine
           fertige Bewertung zugestellt bekommt. */
        '<div id="kpg-abschluss">' + berichtFormularHtml() + '</div>' +
    ...
    bindeBerichtFormular(pollToken);
    postHeight();
  }
