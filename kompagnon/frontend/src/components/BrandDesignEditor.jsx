...
// ── Hilfsfunktion: Hex aufhellen/abdunkeln ───────────────────────────────────
...
const GOOGLE_FONTS = [
  ...
  'Lato', 'Montserrat', 'Poppins', 'Source Sans 3',
  'Bebas Neue', 'Fjalla One', 'DM Sans', 'Nunito',
];
...
export default function BrandDesignEditor({ leadId, token, brandData, onSaved }) {
  ...
  // Brand-Daten beim Mount laden (überschreibt brandData-Prop-Defaults)
  useEffect(() => {
  ...
  return (
    <div ref={editorRef} style={{ fontFamily: 'var(--font-sans)' }}>
      ...
      <div style={{ display: 'flex', borderBottom: '0.5px solid var(--border-light)', marginBottom: 16 }}>
        {[['design','Brand Design'],['guideline','Guideline']].map(([id, lbl]) => (
          <button key={id} onClick={() => setActiveTab(id)} style={{
            ...
            borderBottom: activeTab === id ? '2px solid var(--brand-accent, #FAE600)' : '2px solid transparent',
      ...
      {activeTab === 'design' && (
        <>
          {/* Mini-Vorschau (immer sichtbar, aktualisiert sich live) */}
          <div style={{ border: '0.5px solid var(--border-light)', borderRadius: 'var(--radius-lg)', overflow: 'hidden', marginBottom: 16 }}>
            <div style={{ background: primary, padding: '10px 16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              ...
              <button style={{ background: accent, color: '#fff', border: 'none', borderRadius: radius, padding: '6px 14px', fontSize: 12, fontWeight: 700, cursor: 'default' }}>
                Anfragen
              </button>
            </div>
            <div style={{ background: adjustColor(primary, -20), padding: '16px' }}>
              <div style={{ fontFamily: fontHead, fontSize: 18, fontWeight: 700, color: '#fff', marginBottom: 6 }}>
                &Uuml;berschrift H1 &mdash; Ihr Handwerksbetrieb
              </div>
              <div style={{ fontFamily: fontBody, fontSize: 13, color: 'rgba(255,255,255,.75)', lineHeight: 1.6 }}>
                Flie&szlig;text mit dem gew&auml;hlten Body-Font. Klar, lesbar, professionell.
              </div>
            </div>
            <div style={{ background: '#F5F5F0', padding: '12px 16px' }}>
              <div style={{
                background: '#fff', border: '0.5px solid #D5D5D0', borderRadius: radius, padding: 12,
                boxShadow: shadow === 'stark' ? '0 4px 16px rgba(0,0,0,.12)' : shadow === 'ohne' ? 'none' : '0 2px 6px rgba(0,0,0,.06)',
              }}>
                <div style={{ fontFamily: fontHead, fontSize: 14, fontWeight: 700, color: secondary, marginBottom: 4 }}>
                  Leistungs-Karte
                </div>
                <div style={{ fontFamily: fontBody, fontSize: 12, color: '#555', lineHeight: 1.5 }}>
                  Karteninhalt mit Sekund&auml;rfarbe als &Uuml;berschrift.
                </div>
              </div>
            </div>
          </div>
          ...
          {/* Inline-Editor klappt unter die Chips */}
          {activeToken && (() => {
            const t = tokens.find(x => x.id === activeToken);
            if (!t || !t.setter) return null;
            return (
              <div style={{ border: '1.5px solid var(--brand-primary)', borderRadius: 10, padding: 14, marginBottom: 12, background: 'var(--bg-surface)' }}>
                ...
                {/* Schnell-Palette nur für Akzent */}
                {t.id === 'accent' && (
                  <div>
                    <div style={{ fontSize: 9, fontWeight: 700, color: 'var(--text-tertiary)', textTransform: 'uppercase', letterSpacing: '.08em', marginBottom: 5 }}>
                      Schnell-Palette
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 16 }}>
            <div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                <div>
                  <div style={{ fontSize: 10, color: 'var(--text-tertiary)', marginBottom: 3 }}>&Uuml;berschriften</div>
                  <select value={fontHead} onChange={e => setFontHead(e.target.value)}
                    style={{ width: '100%', padding: '7px 10px', border: '0.5px solid var(--border-light)', borderRadius: 6, fontSize: 12, background: 'var(--bg-surface)', color: 'var(--text-primary)', fontFamily: fontHead }}>
                    {GOOGLE_FONTS.map(f => <option key={f} value={f}>{f}</option>)}
                  </select>
                  <div style={{ marginTop: 4, fontFamily: fontHead, fontSize: 14, fontWeight: 700, color: 'var(--text-primary)' }}>Aa Bb Cc 123</div>
                </div>
                <div>
                  <div style={{ fontSize: 10, color: 'var(--text-tertiary)', marginBottom: 3 }}>Flie&szlig;text</div>
                  <select value={fontBody} onChange={e => setFontBody(e.target.value)}
                    style={{ width: '100%', padding: '7px 10px', border: '0.5px solid var(--border-light)', borderRadius: 6, fontSize: 12, background: 'var(--bg-surface)', color: 'var(--text-primary)', fontFamily: fontBody }}>
                    {GOOGLE_FONTS.map(f => <option key={f} value={f}>{f}</option>)}
                  </select>
                  <div style={{ marginTop: 4, fontFamily: fontBody, fontSize: 12, color: 'var(--text-secondary)', lineHeight: 1.5 }}>
                    Beispiel Flie&szlig;text &mdash; lesbar und klar.
                  </div>
                </div>
              </div>
            </div>
              ...
              <div style={{ display: 'flex', gap: 5, marginBottom: 12 }}>
                {[{label:'Eckig',v:0},{label:'Rund',v:6},{label:'Weich',v:14},{label:'Pill',v:99}].map(r => (
                  <button key={r.v} onClick={() => setRadius(r.v)} style={{
                    ...
                    borderRadius: 6,
                    background: radius === r.v ? 'var(--info-bg, #E0F4F8)' : 'var(--bg-surface)',
                    cursor: 'pointer', fontFamily: 'var(--font-sans)',
                  }}>
      ...
      {activeTab === 'guideline' && (
        <GuidelineTab
          ...
          fontHead={fontHead} fontBody={fontBody} radius={radius} shadow={shadow}
          leadId={leadId} authToken={token}
        />
      )}
    </div>
  );
...
// ── Guideline-Tab ─────────────────────────────────────────────────────────────
function GuidelineTab({ primary, secondary, accent, fontHead, fontBody, radius, shadow, leadId, authToken }) {
  ...
  }, [leadId, authToken]); // eslint-disable-line

  const generate = async () => {
    setGenerating(true);
    try {
      const res = await fetch(`${API_BASE_URL}/api/branddesign/${leadId}/guideline/generate`, {
        method: 'POST',
  ...
  if (loading) return <div style={{ padding: 20, color: 'var(--text-tertiary)', fontSize: 13 }}>Lädt…</div>;
  ...
  if (!guideline) return (
    <div style={{ padding: '20px 0', textAlign: 'center' }}>
      ...
      <button onClick={generate} disabled={generating} style={{
        ...
        padding: '10px 22px', fontSize: 12, fontWeight: 900, cursor: 'pointer', fontFamily: 'var(--font-sans)',
      }}>
  ...
  return (
    <div>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(80px, 1fr))', gap: 8, marginBottom: 16 }}>
        {Object.entries(guideline.colors || {}).slice(0, 12).map(([tk, hex]) => (
          <div key={tk} style={{ textAlign: 'center' }}>
            <div onClick={() => navigator.clipboard?.writeText(hex)} title={`${tk}: ${hex} — klick zum kopieren`}
      ...
      <div style={{ border: '0.5px solid var(--border-light)', borderRadius: 8, padding: '12px 14px', marginBottom: 12 }}>
        <div style={{ fontSize: 9, fontWeight: 700, color: 'var(--text-tertiary)', textTransform: 'uppercase', letterSpacing: '.08em', marginBottom: 8 }}>Typografie</div>
        <div style={{ fontFamily: fontHead, fontSize: 18, fontWeight: 700, color: primary, marginBottom: 4 }}>H1 Schrift: {fontHead}</div>
        <div style={{ fontFamily: fontHead, fontSize: 14, fontWeight: 600, color: secondary, marginBottom: 8 }}>H2 Sekundärüberschrift</div>
        <div style={{ fontFamily: fontBody, fontSize: 12, color: '#555', lineHeight: 1.7 }}>
          Fließtext mit {fontBody} &mdash; lesbar, professionell, überzeugend.
        </div>
      </div>
      <div style={{ padding: '8px 12px', background: 'var(--bg-app)', borderRadius: 8, fontSize: 11, color: 'var(--text-secondary)', lineHeight: 1.6 }}>
        Vollständige Guideline → im nächsten Schritt &ldquo;Brand Guideline&rdquo; verfügbar.
      </div>
    </div>
  );
}
