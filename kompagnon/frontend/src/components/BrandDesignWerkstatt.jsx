  ...
  const suggestFonts = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        toast.success('KI-Font-Vorschläge geladen');
      }
    } catch { toast.error('Font-Vorschläge konnten nicht geladen werden'); }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16, padding: '16px 24px' }}>
      ...
      <div style={{
        ...
        border: `0.5px solid ${brand?.scraped_at ? '#00875A33' : 'var(--border-light)'}`,
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
        ...
        <div style={{ background: 'var(--bg-surface)', border: '0.5px solid var(--border-light)', borderRadius: 10, padding: 16 }}>
          ...
          {[
            { label: 'Primärfarbe',   value: primaryColor,   set: setPrimaryColor   },
            { label: 'Sekundärfarbe', value: secondaryColor, set: setSecondaryColor },
              ...
              <div style={{ marginTop: 8 }}>
                ...
                {fontSuggestions.map(f => (
                  <div key={f.name} onClick={() => {
                    ...
                    padding: '7px 10px', borderRadius: 6, border: '1px solid var(--border-light)',
                    marginBottom: 5, cursor: 'pointer', background: 'var(--bg-app)',
        ...
        <div style={{ padding: 16 }}>
          <div style={{ border: '0.5px solid var(--border-light)', borderRadius: parseInt(borderRadius) > 0 ? 12 : 0, overflow: 'hidden' }}>
            <div style={{ background: primaryColor, padding: '14px 18px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <span style={{ fontFamily: fontPrimary ? `'${fontPrimary}', serif` : 'serif', fontSize: 16, fontWeight: 700, color: '#fff' }}>
                {companyName || 'Firmenname'}
              </span>
              <button style={{ background: accentColor, color: '#000', border: 'none', borderRadius: borderRadius, padding: '7px 14px', fontSize: 12, fontWeight: 700, cursor: 'default' }}>
                Kontakt aufnehmen
              </button>
            </div>
            <div style={{ padding: '16px 18px', background: '#fff' }}>
              <h2 style={{ fontFamily: fontPrimary ? `'${fontPrimary}', serif` : 'serif', fontSize: 20, fontWeight: 700, color: secondaryColor, marginBottom: 8, marginTop: 0 }}>
                Headline der neuen Website
              </h2>
              <p style={{ fontFamily: fontSecondary ? `'${fontSecondary}', sans-serif` : 'sans-serif', fontSize: 13, color: '#555', lineHeight: 1.6, marginBottom: 14 }}>
                Beschreibungstext mit dem gewählten Fließtext-Font. Professionell, klar und auf die Zielgruppe abgestimmt.
              </p>
              <button style={{ background: primaryColor, color: '#fff', border: 'none', borderRadius: borderRadius, padding: '9px 18px', fontSize: 12, fontWeight: 700, cursor: 'default' }}>
                Jetzt anfragen →
              </button>
            </div>
          </div>
