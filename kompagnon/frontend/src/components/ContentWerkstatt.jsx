...
export default function ContentWerkstatt({ project, sitemapPages, sitemapLoading, token, leadId, websiteContent, onProjectRefresh, mode }) {
  const [activeTab, setActiveTab]       = useState(mode || 'inhalte');
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
      ...
      {/* Unter-Tab-Navigation — nur anzeigen wenn kein mode gesetzt (freie Navigation) */}
      {!mode && (
                    ...
                    onClick={async () => {
                      ...
                      try {
                        ...
                        setPageContent(prev => ({
                          ...prev,
                          [selectedPage.id]: {
                            headline: data.ground_page.page_title,
                            subheadline: data.ground_page.meta_description,
                            intro: data.ground_page.intro,
                            content: JSON.stringify(data.ground_page, null, 2),
                            meta_description: data.ground_page.meta_description,
