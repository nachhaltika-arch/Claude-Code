      ...
      {showAdd && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center' }} onClick={e => e.target === e.currentTarget && setShowAdd(false)}>
          <div style={{ background: 'var(--bg-surface)', borderRadius: 12, padding: 28, width: '100%', maxWidth: 440, display: 'flex', flexDirection: 'column', gap: 12 }}>
            ...
            <div><label style={labelS}>Bezeichnung</label><input aria-label="Bezeichnung" style={inputS} value={form.label} onChange={e => setForm(f => ({ ...f, label: e.target.value }))} placeholder="z.B. IONOS Hosting, WordPress Admin" /></div>
            <div><label style={labelS}>Benutzername</label><input aria-label="Benutzername" style={inputS} value={form.username} onChange={e => setForm(f => ({ ...f, username: e.target.value }))} /></div>
            <div><label style={labelS}>Passwort</label><input aria-label="Passwort" style={inputS} type="password" value={form.password} onChange={e => setForm(f => ({ ...f, password: e.target.value }))} /></div>
            <div><label style={labelS}>URL</label><input aria-label="URL" style={inputS} value={form.url} onChange={e => setForm(f => ({ ...f, url: e.target.value }))} placeholder="https://..." /></div>
            <div><label style={labelS}>Notizen</label><textarea aria-label="Notizen" style={{ ...inputS, minHeight: 50, resize: 'vertical' }} value={form.notes} onChange={e => setForm(f => ({ ...f, notes: e.target.value }))} /></div>
