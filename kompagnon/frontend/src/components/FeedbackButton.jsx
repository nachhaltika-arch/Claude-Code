...
const TYPES = [
  ...
  { id: 'feedback', label: 'Feedback geben', color: 'var(--brand-primary)' },
...
export default function FeedbackButton() {
  ...
  const inp = { width: '100%', padding: '10px 12px', border: '1px solid var(--border-light)', borderRadius: 'var(--radius-md)', fontSize: 14, boxSizing: 'border-box', outline: 'none' };
      ...
      {open && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', zIndex: 200, display: 'flex', alignItems: 'flex-end', justifyContent: 'flex-end', padding: isMobile ? 0 : 24 }}
          ...
          <div style={{ background: 'var(--bg-surface)', borderRadius: isMobile ? '16px 16px 0 0' : 16, width: '100%', maxWidth: 420, boxShadow: '0 20px 60px rgba(0,0,0,0.2)', overflow: 'hidden', maxHeight: isMobile ? '90vh' : 'auto', overflowY: 'auto' }}>
            {step === 'form' ? (
              <>
                <div style={{ background: 'var(--brand-primary)', padding: '16px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  <div>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6, marginBottom: 16 }}>
                    {TYPES.map((t) => (
                      <button key={t.id} onClick={() => setForm((p) => ({ ...p, type: t.id }))} style={{
                        padding: '8px 10px', borderRadius: 'var(--radius-md)', border: `1.5px solid ${form.type === t.id ? t.color : '#e2e8f0'}`,
