...
function KiEmpfehlung({ leadId, token, onAdded }) {
  ...
  const laden = async () => {
    ...
    try {
      ...
      if (res.ok) setEmpfehlungen(await res.json());
    } catch { /* silent */ }
  ...
  const addPage = async (page) => {
    ...
    try {
      ...
      if (res.ok) {
        toast.success(`"${page.page_name}" hinzugefuegt`);
      ...
      <div style={{
        ...
        <div>
          <div style={{
            ...
            KI-Empfehlung {empfehlungen?.company ? `fuer ${empfehlungen.company}` : ''}
          </div>
      ...
      {bedingte.length > 0 && (
        <div style={{ marginBottom: 16 }}>
          <div style={{
            ...
            Bedingte Pflichtseiten — unter Umstaenden gesetzlich erforderlich
          </div>
          {bedingte.map(p => (
