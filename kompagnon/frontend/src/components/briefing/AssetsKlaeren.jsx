...
export default function AssetsKlaeren({ leadId, token, onSaved }) {
  ...
  if (loading) return <div style={{ padding: 20, color: 'var(--text-tertiary)', fontSize: 13 }}>Erkenne Assets…</div>;
  ...
  return (
    <div style={{ fontFamily: 'var(--font-sans)' }}>
          ...
          status?.logo?.url ? (
            <img
              ...
              style={{ height: 32, maxWidth: 120, objectFit: 'contain',
                       background: '#f0f0f0', borderRadius: 4, padding: 4 }}
      ...
      <AssetRow
            ...
            <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap', marginTop: 6 }}>
              {status.fotos.vorschau.slice(0, 3).map((src, i) => (
                <img key={i} src={src} alt=""
                  style={{ width: 48, height: 48, objectFit: 'cover', borderRadius: 4, border: '0.5px solid var(--border-light)' }}
                  onError={e => { e.target.style.display = 'none'; }}
                />
              ))}
              {status.fotos.anzahl > 3 && (
                <div style={{ width: 48, height: 48, borderRadius: 4, background: 'var(--surface)',
                              border: '0.5px solid var(--border-light)', display: 'flex',
        ...
        uncheckedHint="Kein Fotograf? Handy-Fotos als Platzhalter, professionelle Fotos empfohlen"
      />
