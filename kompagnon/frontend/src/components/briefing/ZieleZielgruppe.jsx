...
const CTA_OPTIONS = ['Anrufen','Kontaktformular ausfüllen','WhatsApp schreiben',
...
export default function ZieleZielgruppe({ leadId, token, briefing, onSaved }) {
  ...
  const isKi = (field) => ki && ki[field] === data[field] && ki[field + '_konfidenz'] > 0.7;
  ...
  return (
    <div style={{ fontFamily: 'var(--font-sans)' }}>
      ...
      {loading && (
        <div style={{ padding: '12px 16px', background: '#E0F4F8', borderRadius: 8, marginBottom: 16, fontSize: 12, color: '#004F59' }}>
          KI analysiert Website-Daten und füllt Felder vor…
        </div>
      )}
      {ki && !loading && (
        <div style={{ padding: '10px 14px', background: '#E3F6EF', border: '0.5px solid #00875A33', borderRadius: 8, marginBottom: 16, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
          <div style={{ fontSize: 12, color: '#00875A' }}>
            KI hat alle Felder vorausgefüllt — {ki.ki_begruendung}
          </div>
          <button onClick={loadKi} style={{ fontSize: 10, fontWeight: 700, background: 'transparent', border: '0.5px solid #00875A44', borderRadius: 5, padding: '3px 9px', cursor: 'pointer', color: '#00875A', fontFamily: 'var(--font-sans)' }}>
            Neu
          </button>
        </div>
      )}
      ...
      ].map(({ key, label, type, placeholder, hint, options }) => (
        <div key={key} style={{ marginBottom: 14 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 5 }}>
            <label style={{ fontSize: 11, fontWeight: 700, color: 'var(--text-secondary)', textTransform: 'uppercase', letterSpacing: '.06em' }}>
              {label}
            </label>
            {isKi(key) && (
              <span style={{ fontSize: 9, fontWeight: 900, padding: '1px 6px', borderRadius: 3, background: '#E3F6EF', color: '#00875A' }}>
                KI · {Math.round(konfidenz(key) * 100)}%
              </span>
            )}
          </div>
          {type === 'textarea' ? (
            <textarea
              ...
              style={{
                ...
                border: isKi(key) ? '1.5px solid #00875A55' : '1px solid var(--border-light)',
                borderRadius: 8, fontSize: 13, fontFamily: 'var(--font-sans)',
                background: isKi(key) ? '#F0FDF4' : 'var(--bg-surface)',
      ...
      <div style={{ display: 'flex', gap: 8, marginTop: 8 }}>
        <button
          ...
          disabled={saving}
          style={{
            flex: 1, padding: '12px', background: '#FAE600', color: '#000',
        ...
        <button
          ...
          disabled={saving}
          style={{
            padding: '12px 16px', background: 'transparent', color: 'var(--text-tertiary)',
