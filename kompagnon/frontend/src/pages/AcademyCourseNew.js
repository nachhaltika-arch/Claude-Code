...
export default function AcademyCourseNew() {
  ...
  const isCompleted = (lessonId) => progressMap[lessonId]?.completed;

  // Linear locking logic
  const isLocked = (lessonId) => {
    if (!course?.linear_progress) return false;
    const idx = allLessons.findIndex(l => l.id === lessonId);
    if (idx === 0) return false;
    for (let i = 0; i < idx; i++) {
      if (!progressMap[allLessons[i].id]?.completed) return true;
    }
    return false;
  };
  ...
  const handleComplete = async () => {
    ...
    try {
      ...
      setProgressMap(prev => ({
        ...prev,
        [activeLessonId]: { ...prev[activeLessonId], completed: data.completed },
      }));
  ...
  const handleSubmitQuiz = async () => {
    ...
    try {
      ...
      if (data.passed) {
        setProgressMap(prev => ({
          ...prev,
          [activeLessonId]: { ...prev[activeLessonId], completed: true, score: data.score },
        }));
      }
    } catch (e) { console.error(e); }
  ...
  return (
    <div style={{ display: 'flex', gap: 0, minHeight: '70vh', position: 'relative' }}>
      ...
      <aside style={{
        ...
        <div style={{ padding: '16px 16px 12px', borderBottom: '1px solid var(--border-light)' }}>
          <button onClick={() => navigate('/app/akademie')} style={{
            ...
            fontSize: 12, cursor: 'pointer', padding: 0, marginBottom: 10, fontFamily: 'var(--font-sans)',
          }}>← Zur Übersicht</button>
          <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--text-primary)', lineHeight: 1.4, marginBottom: 10 }}>{course.title}</div>
          {/* Overall progress */}
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
            <span style={{ fontSize: 10, color: 'var(--text-tertiary)' }}>{completedCount} / {totalLessons} Lektionen</span>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: 8 }}>
          {modules.map(mod => {
            ...
            const modDone = modLessons.filter(l => isCompleted(l.id)).length;
            const modOpen = openModules[mod.id];
            return (
              <div key={mod.id} style={{ marginBottom: 4 }}>
                {/* Module toggle */}
                <button onClick={() => setOpenModules(p => ({ ...p, [mod.id]: !p[mod.id] }))} style={{
                  width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'space-between',
                  padding: '9px 10px', background: modOpen ? 'var(--bg-app)' : 'transparent',
                  border: 'none', borderRadius: 'var(--radius-md)', cursor: 'pointer',
                  color: 'var(--text-primary)', textAlign: 'left',
                }}>
                  <span style={{ fontSize: 12, fontWeight: 600, flex: 1 }}>{mod.title}</span>
                  <span style={{ fontSize: 10, color: 'var(--text-tertiary)', marginRight: 6, flexShrink: 0 }}>{modDone}/{modLessons.length}</span>
                  <span style={{ fontSize: 10, color: 'var(--text-tertiary)', transform: modOpen ? 'rotate(180deg)' : 'none', transition: 'transform 0.2s' }}>▼</span>
                </button>
                {/* Lessons */}
                {modOpen && (
                  <div style={{ paddingLeft: 8 }}>
                    {modLessons.map(lesson => {
                      ...
                      return (
                        <button key={lesson.id} onClick={() => handleSelectLesson(lesson)} disabled={locked} style={{
                          width: '100%', display: 'flex', alignItems: 'center', gap: 8, padding: '7px 10px',
                          background: active ? 'var(--brand-primary)' : 'transparent',
                          border: 'none', borderRadius: 'var(--radius-md)',
                          cursor: locked ? 'not-allowed' : 'pointer',
                          color: active ? 'white' : locked ? 'var(--text-tertiary)' : 'var(--text-secondary)',
                          textAlign: 'left', opacity: locked ? 0.55 : 1,
                        }}>
                          <span style={{ fontSize: 13, flexShrink: 0 }}>
                            {locked ? '🔒' : done ? '✅' : active ? '▶' : '○'}
                          </span>
                          <span style={{ fontSize: 12, lineHeight: 1.3, flex: 1 }}>{lesson.title}</span>
                          <span style={{ fontSize: 11, flexShrink: 0, opacity: 0.7 }}>{TYPE_ICON[lesson.type] || '📄'}</span>
                        </button>
                      );
                    })}
                  </div>
      ...
      {/* Main Content */}
      <div style={{ flex: 1, minWidth: 0, marginLeft: 20, display: 'flex', flexDirection: 'column', gap: 16 }}>

        {/* Mobile sidebar trigger */}
        <button onClick={() => setSidebarOpen(true)} style={{
          display: 'none', padding: '8px 14px', background: 'var(--bg-surface)',
          border: '1px solid var(--border-medium)', borderRadius: 'var(--radius-md)',
          fontSize: 12, cursor: 'pointer', alignItems: 'center', gap: 8, fontFamily: 'var(--font-sans)',
          // shown on mobile via CSS
        }}>☰ Kursinhalt</button>
        ...
        {activeLesson ? (
          <>
            ...
            {/* Video */}
            {(activeLesson.type === 'video' || activeLesson.video_url) && activeLesson.video_url && (
            ...
            {/* Text content */}
            {activeLesson.content_text && activeLesson.type !== 'quiz' && (
              <div style={{
                background: 'var(--bg-surface)', border: '1px solid var(--border-light)',
            ...
            {activeLesson.type === 'quiz' && (
              <div style={{ background: 'var(--bg-surface)', border: '1px solid var(--border-light)', borderRadius: 'var(--radius-lg)', padding: 24 }}>
                <div style={{ fontSize: 15, fontWeight: 700, color: 'var(--text-primary)', marginBottom: 20 }}>
                  ❓ Quiz — {quizQuestions.length} Fragen
                </div>
                {quizQuestions.length === 0 ? (
                  ...
                  <div style={{ textAlign: 'center', padding: '20px 0' }}>
                    ...
                    <div style={{ fontSize: 24, fontWeight: 800, color: quizResult.passed ? '#16a34a' : '#dc2626', marginBottom: 8 }}>
                      {quizResult.score}%
                    </div>
                    ...
                    <div style={{ fontSize: 13, fontWeight: 600, color: quizResult.passed ? '#16a34a' : '#dc2626', marginBottom: 24 }}>
                      {quizResult.passed ? '✓ Bestanden' : '✗ Nicht bestanden — Mindestpunktzahl 70%'}
                    </div>
                    {!quizResult.passed && (
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                          {q.answers.map(ans => (
                            <label key={ans.id} style={{
                              ...
                              background: quizAnswers[q.id] === ans.id ? 'var(--brand-primary)15' : 'transparent',
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingTop: 4 }}>
              <button onClick={() => prevLesson && setActiveLessonId(prevLesson.id)} disabled={!prevLesson} style={{
                padding: '9px 18px', background: 'var(--bg-surface)', color: 'var(--text-secondary)',
                border: '1px solid var(--border-medium)', borderRadius: 'var(--radius-md)',
                fontSize: 12, fontWeight: 600, cursor: prevLesson ? 'pointer' : 'default',
              ...
              {activeLesson.type !== 'quiz' && (
                <button onClick={handleComplete} disabled={completing} style={{
                  padding: '9px 20px',
                  background: isCompleted(activeLessonId) ? '#f0fdf4' : 'var(--brand-primary)',
                  color: isCompleted(activeLessonId) ? '#16a34a' : 'white',
                  border: isCompleted(activeLessonId) ? '1px solid #bbf7d0' : 'none',
                  borderRadius: 'var(--radius-md)', fontSize: 12, fontWeight: 700,
                  cursor: 'pointer', fontFamily: 'var(--font-sans)',
                }}>
                  {isCompleted(activeLessonId) ? '✓ Als offen markieren' : completing ? 'Speichern…' : 'Als erledigt markieren →'}
                </button>
              )}

              <button onClick={() => nextLesson && setActiveLessonId(nextLesson.id)} disabled={!nextLesson} style={{
                padding: '9px 18px', background: nextLesson ? 'var(--brand-primary)' : 'var(--bg-surface)',
                ...
                cursor: nextLesson ? 'pointer' : 'default', fontFamily: 'var(--font-sans)',
            ...
            {overallPct === 100 && (
              <div style={{
                ...
                <div>
                  ...
                  <div style={{ fontSize: 12, color: '#15803d' }}>Du kannst jetzt dein Zertifikat herunterladen.</div>
                </div>
                <button
                  onClick={() => fetch(`${API_BASE_URL}/api/academy/courses/${kursId}/certificate`, {
          ...
          <div style={{ textAlign: 'center', padding: '60px 20px', color: 'var(--text-tertiary)' }}>
            ...
            <div style={{ fontSize: 14 }}>Wähle eine Lektion aus der Sidebar aus.</div>
          </div>
        )}
      </div>
    </div>
