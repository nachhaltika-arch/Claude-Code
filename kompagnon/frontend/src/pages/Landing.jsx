  ...
  return (
    <div style={{ fontFamily: 'system-ui, -apple-system, sans-serif', color: N, overflowX: 'hidden' }}>
      ...
      <section style={{ background: '#fff', padding: isMobile ? '48px 20px' : '72px 60px' }}>
        <SectionHead title="Transparent. Fair. Festpreis." />
        <div style={{ display: 'flex', gap: 20, justifyContent: 'center', flexDirection: isMobile ? 'column' : 'row', alignItems: 'center', maxWidth: 960, margin: '0 auto' }}>
          <PriceCard name="Starter" price="1.500" features={['5 Seiten', 'SEO Basic', 'Mobile-optimiert', 'DSGVO-konform']} missing={['GEO-Optimierung', 'Strategie-Workshop']} onClick={() => go('starter')} />
          <PriceCard name="Kompagnon" price="2.000" recommended features={['8 Seiten', 'SEO Komplett', 'GEO-Optimierung', 'Strategie-Workshop', '4 Wochen Nachbetreuung']} onClick={() => go('kompagnon')} />
          <PriceCard name="Premium" price="2.800" features={['12 Seiten', 'SEO + GEO Komplett', 'Shop-Ready', 'Professionelles Fotoshooting', '3 Monate Betreuung']} onClick={() => go('premium')} />
        </div>
        <p style={{ textAlign: 'center', fontSize: 13, color: G, marginTop: 24 }}>
          Alle Preise zzgl. 19% MwSt. · Zahlbar per Vorkasse · Keine versteckten Kosten
        </p>
      </section>
