...
export default function ProjectDetail() {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
              ...
              const runAllScans = async () => {
                ...
                for (let i = 0; i < SCAN_STEPS.length; i++) {
                  ...
                  try {
                    ...
                    if (SCAN_STEPS[i].key === 'fonts' && result?.suggestions) setFontSuggestions(result.suggestions);
                  } catch {
      ...
      {/* ── Analyse-Zentrale Tab ──────────────────────────────────────────── */}
      {(activeSubTab === 'analyse' || activeTab === 'analyse') && (
        <AnalyseCentrale
          projectId={id}
          leadId={project.lead_id}
          websiteUrl={project.website_url || lead?.website_url}
          token={token}
        />
      )}
